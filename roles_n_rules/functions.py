...
def myrole(user_id=message.author.id):
    if game_master in [y.id for y in message.author.roles]:
        return Mailbox().respond("The role of user <@{}> is {}".format(user_id, db.db_get(user_id,'role'))
    else if game_master not in [y.id for y in message.author.roles] and user_id != message.author.id:
        return Mailbox().respond("Hey stop that! You're not a game master! No peeking at anyone else's roles!")
    else:
        return Mailbox().respond("Your role is {}".format(db.db_get(message.author.id,'role'))
