...
from config import welcome_channel, game_master, administrator, peasant
...
async def remove_all_game_roles(member: Member):
    await member.remove_roles(
        *[role for role in member.roles if
          role.id in [config.frozen_participant, config.participant, config.suspended]],
        reason="Updating CC permissions")
...
async def on_message(message):
    ...
    result = await process(message, isGameMaster, isAdmin, isPeasant, client)
    ...
    for mailbox in result:
            ...
            for poll in db.get_all_polls():
                ...
                if poll_channel == None:
                    await botspam_channel.send(
                        "We got a problem! Could you send these results to the appropriate channel, please?")
        ...
        for element in mailbox.answer:
            if element.embed:
                msg = await message.channel.send(embed=element.content)
            else:
        ...
        for element in mailbox.player:
            ...
            if member == None:
                ...
                try:
                    ...
                    botspam_channel.send(
                        'I wasn\'t allowed to send a DM to <@{}>! Here\'s the content:'.format(member.id))
                    botspam_channel.send(element.content)
                except Exception:
                    botspam_channel.send(
                        'I failed to send a DM to <@{}>! Could somebody send this, please?'.format(member.id))
                    botspam_channel.send(element.content)
        ...
        for element in mailbox.oldchannels:
            ...
            if element.number == 0:
                ...
                await member.add_roles(get_role(main_guild.roles, config.frozen_participant),
                                       reason="Updating CC Permissions")
                ...
                await channel.set_permissions(user, read_messages=True, send_messages=False)
                await member.add_roles(get_role(main_guild.roles, config.dead_participant),
                                       reason="Updating CC Permissions")
        ...
        for element in mailbox.newchannels:
            ...
            if ' ' not in element.name:
                ...
                for user in element.members:
                    ...
                    if member == None:
                        await message.author.send(
                            "It doesn't seem like <@{}> is part of the server! I am sorry, I can't add them to your **conspiracy channel**.".format(
                                user))
                ...
                default_permissions = {
                    ...
                    **{
                        member: discord.PermissionOverwrite(read_messages=True, send_messages=True) for member in
                        viewers
                    },
                    **{
                        member: discord.PermissionOverwrite(read_messages=True, send_messages=False) for member in
                        frozones
                    },
                    **{
                        member: discord.PermissionOverwrite(read_messages=True, send_messages=False) for member in
                ...
                if not element.secret:
                    ...
                    category_name = 'S{} Secret Channels Part {}'.format(config.season,
                                                                         db.count_categories(element.secret) + 1)

                # Create a new category if needed
                if db.get_category(element.secret) == None:
                    category = await main_guild.create_category(category_name,
                                                                reason='It seems like we couldn\'t use our previous category! Don\'t worry, I just created a new one.')
